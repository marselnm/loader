...
/* \brief Configure SPI								*/
/*------------------------------------------------------------------------------*/
// pcs = AT91C_SPI_PCS_DATAFLASH = 0xE
	...
	unsigned int size)
{
	...
	volatile unsigned 	char	temp_buf[AT91C_MAX_PDC_SIZE];
	unsigned int * pInt;
	volatile unsigned 	int *pS;
	unsigned int i;
	
	pS = (volatile unsigned int*)(buffer); // 0x10000000
	...
	while (size)
	{
		...
		df_wait_ready(pDf);
			df_continuous_read(pDf, (char *)temp_buf, SizeToRead, addr);
	    	
			pInt = (unsigned int*)temp_buf;
			
	    	for( i = 0; i < (SizeToRead/4); i++) {
				*(pS++) = *(pInt++);
			}
